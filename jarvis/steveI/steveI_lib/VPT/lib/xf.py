...
from jarvis.steveI.steveI_lib.VPT.lib import misc, mlp
from jarvis.steveI.steveI_lib.VPT.lib import torch_util as tu
from jarvis.steveI.steveI_lib.VPT.lib import util
...
def attention(
    ...
    if isinstance(mask, th.Tensor):
        bias = ((~mask) * -1e9).to(Q_bte.dtype)
    elif mask:
        bias = get_attn_bias_cached(Q_bte.shape[1], K_bTe.shape[1], maxlen=maxlen, device=Q_bte.device, dtype=Q_bte.dtype)
    else:
        bias = Q_bte.new_zeros((), dtype=Q_bte.dtype)
    ...
    if check_sentinel:
        logit_btT = logit_btT - 1e9 * invalid
    W_btT = th.softmax(logit_btT, dim=2).to(Q_bte.dtype)
    if callable(V_bTe):
...
class SelfAttentionLayer(AttentionLayerBase):
    ...
    def initial_state(self, batchsize, initial_T=0):
        # Set the datatype based on the parameters
        parameters = self.parameters()
        for p in parameters:
            dtype = p.dtype
            break
        return (
            tu.zeros((batchsize, initial_T, self.x_size), dtype=dtype),
            tu.zeros((batchsize, initial_T, self.x_size), dtype=dtype),
        )
